...
# 문자열 개수 세기 (count)
str1 = "hobby"
print(str1.count('b')) # str1 에 들어있는 b 갯수를 출력 시키기

# 문자열 위치 알려주기1 (find)
print(str1.find('b')) # str1 에 들어있는 b 중 제일 앞에 있는 인덱스를 찾아 출력 해준다.
print(str1.find('x')) # str1 에 들어있지 않은 x 를 찾게 하면 -1 을 출력한다.

# 문자열 위치 알려주기2 (index)
# print(str1.index('x')) # str1 에 들어있지 않은 x 를 찾게 하면 index 메서드에서는 오류를 내보낸다.

# 문자열 샆입하기 (join)
str2 = ",".join('abcd') # str2 라는 변수를 선언하고 "abcd" 사이사이 마다 "," 를 집어 넣어준다.
print(str2)
str3 = ", ".join('abcd') # 이번에는 "," 뒤에 " " 을 추가해주면 ", " 를 사이사이마다 추가해준다.
print(str3)

# 문자열 소문자 대문자로 바꾸기 (upper)
str4 = "hello"
print(str4.upper()) # upper 메서드는 소문자를 전부 대문자로 변경해준다.
# 문자열 대문자를 소문자로 바꾸기 (lower)
str5 = "PYTON" 
print(str5.lower()) # lower 메서드는 대문자를 전부 소문자로 변경해준다.

# 문자열 공백 지우기 (띄어쓰기 갯수 상관없이 제일 외곽만 지운다.)
str6 = " hi "
# 문자열의 제일 왼쪽 공백 지우기 (lstrip())
print(str6.lstrip()) # 제일 왼쪽 공백만 지워진다.
# 문자열의 제일 오른쪽 공백 지우기 (rstrip())
print(str6.rstrip()) # 제일 오른쪽 공백만 지워진다.
# 양쪽 제일 외각 공백을 지우기 (strip())
print(str6.strip()) # 외각 공백만 지운다 내부에 공백이 있는것은 안지운다.

# 문자열 대치하기 (replace())
str7 = "Life is too short"
print(str7.replace("Life", "Your leg")) # replace 명령어는 ("대상문자", "대치될 문자") 로 사용하면 된다.

# 문자열 나누기 (split())
print(str7.split()) # 스플릿을 쓰게 되면 배열로 저장되게 된다. split() 의 내부에 아무것도 안쓰면 기본적으로 공백을 기준으로 나눈다.
print(str7.split('e')) # e 를 넣어주니 e 를 기준으로 자르게 되는데 e 는 생략 하고 나눠주나보다.
